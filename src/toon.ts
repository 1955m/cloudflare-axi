...
export function extract<T>(item: T, schema: FieldDef<T>[]): Item {
  ...
  for (const def of schema) {
    const outputKey = def.as ?? def.key ?? "";
    switch (def.type) {
      case "field":
...
function formatRelativeTime(iso: string | number | undefined | null): string {
  if (iso === undefined || iso === null || iso === "") return "unknown";
  // Cloudflare returns ISO 8601 timestamps (created_on/modified_on); audit logs
  // can return epoch-ms. Treat purely-numeric values as epoch ms, otherwise
  // fall back to Date parsing (ISO 8601). Mirrors the clickup-axi toon helper.
